...
export function RainWindowExperience({
  isFullscreen,
  language,
  updateLanguage,
  headerEyebrow,
  ...
  return (
    <div className={`rain-page ${isFullscreen ? "is-fullscreen-view" : ""}`}>
      ...
      <section className="rain-mood-panel">
        <label>
        ...
        <div className="time-jump-row rain-mood-presets">
          <button type="button" className="focus-chip" onClick={() => { setRainIntensity(0.42); setRainFlow(0.34); }}>
            {language === "ko" ? "가벼운 비" : "Light rain"}
          </button>
          <button type="button" className="focus-chip" onClick={() => { setRainIntensity(0.56); setRainFlow(0.42); }}>
            {language === "ko" ? "기본 감상" : "Balanced"}
          </button>
          <button type="button" className="focus-chip" onClick={() => { setRainIntensity(0.74); setRainFlow(0.58); }}>
            {language === "ko" ? "짙은 빗소리" : "Deep rain"}
          </button>
        </div>
      </section>

      <section className="viewer-overlay aurora-live-controls rain-bottom-controls">
